...
class Switch(StpSwitch):
    ...
    def remove_active_link(self, switchID):
        if switchID in self.activeLinks:
            self.activeLinks.remove(switchID)
        return

    def message_neighbors(self):
        for link in self.links:
            if link == self.throughSwitch:
                msg = Message(self.root, self.distance, self.switchID, link, True)
            else:
                msg = Message(self.root, self.distance, self.switchID, link, False)
        ...
        if message.root == self.root:
            ...
            if message.distance + 1 == self.distance:
                if message.origin < self.throughSwitch:
                    ...
                    self.throughSwitch = message.origin
                    self.add_active_link(self.throughSwitch)
